...
rgl.viewpoint(theta = 15, phi = 20, zoom = .7)

plot3d(log10(diamonds$carat), log10(diamonds$price), diamonds$color, 
       size = 2, type = "p", col = c(diamonds$color), 
       xlab = "log(carat)", ylab="log(price)", zlab="color",
           top = TRUE, box = FALSE, aspect = c(16,9,7))       
...
d4ColorSize <- ggplot(diamonds) + aes(log10(carat), log10(price), 
                                    color = cut, size = x) + 
  geom_point(alpha = 0.4) + scale_size(range = c(2, 6)) +
  theme_multi()
d4ColorSize
