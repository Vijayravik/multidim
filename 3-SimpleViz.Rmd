...
write.table(dlarge, file = "dlarge.csv", 
...
dsp <- ggplot(dsmall, aes("price", price)) + 
  geom_point(size = 2, color = I('orange')) + coord_flip() +
  theme_black_clean()
dsp
```
...
dspj <- ggplot(dsmall, aes("price", price)) + 
  geom_point(size = 2, color = I('orange'), 
...
psp <- ggplot(diamonds, aes(x = "", fill = cut)) +
  geom_bar(width = 1)   +
  theme_black() + coord_polar(theta="y")
psp
```
...
pmm <- ggMMplot(diamonds$cut, diamonds$clarity) + theme_black() + 
  xlab("cut") + ylab("clarity")
pmm
```
...
cms <- ggplot(dlarge, aes(carat,price)) +
  geom_point(color = I("orange"), alpha = I(0.8) , size = 3) +
  theme_black() + facet_wrap(~cut)
cms
```


## Timeseries - multiple - series (tss)
```{r time-multiple-series, echo=FALSE}
devices <- read.csv("~/Dropbox/R/diamonds/devices.csv", 
                    header = TRUE)

tms <- ggplot(devices, aes(year,total, color = type)) +
  geom_line(size = 2) + scale_x_continuous(breaks=seq(2006,2017,1)) +
  theme_black() 
tms
```

## Timeseries - multiple - area (tma)
```{r time-multiple-area, echo=FALSE}

tma <- ggplot(devices, aes(year,total, fill = type)) +
  geom_area() + scale_x_continuous(breaks=seq(2006,2017,1)) +
  theme_black() 
tma
```

## Timeseries - multiple - bar (tmb)
```{r time-multiple-bar, echo=FALSE}

tmb <- ggplot(devices, aes(year,total, fill = type)) +
  geom_bar(stat="identity") + scale_x_continuous(breaks=seq(2006,2017,1)) +
  theme_black() 
tmb
```


## Timeseries - multiple - area - fill (tmf)
```{r time-multiple-area-fill, echo=FALSE}

tmaf <- ggplot(devices, aes(year,total, fill = type)) +
...
## Timeseries - multiple - bar - fill (tmf)
```{r time-multiple-bar-fill, echo=FALSE}

tmbf <- ggplot(devices, aes(year,total, fill = type)) +
...
## Timeseries - multiple - area - dodge (tmf)
```{r time-multiple-area-dodge, echo=FALSE}

tmad <- ggplot(devices, aes(year,total, fill = type)) +
...
## Timeseries - multiple - bar - dodge (tmf)
```{r time-multiple-bar-dodge, echo=FALSE}

tmbd <- ggplot(devices, aes(year,total, fill = type)) +
...
## Timeseries - multiple - area - facet (tmf)
```{r time-multiple-area-facet, echo=FALSE}

tmf <- ggplot(devices, aes(year,total, fill = type)) +
  geom_area() + scale_x_continuous(breaks=seq(2006,2017,1)) +
  theme_black() + facet_grid(type~.)
tmf
```

## Timeseries - multiple - index (tmi)
```{r time-multiple-index, echo=FALSE}

tmi <- ggplot(devices, aes(year,index, color = type)) +
  geom_line(size = 2)  +
  theme_black()
tmi
```

## Timeseries - multiple - slopegraph (tmsl)
```{r time-multiple-slope, echo=FALSE}
sdevices <- subset(devices, year == c(2010,2017))
```
